    ...
    case'show':
		if(isset($_POST['id']) && $_POST['id'] > 0 && isset($_POST['num']) && $_POST['num'] > 0){
		  if (isset($_POST['par'])){
            ...
            elseif ($_POST['par'] == 'wlist')
                $_fullList=$COMMENTS->ShowComments(array('wlist_id'=>$_POST['id']),$_POST['num'],0,'ASC');
            else
                $_fullList=$COMMENTS->ShowComments(array('shop_id'=>$_POST['id']),$_POST['num'],0,'ASC');
          }
          else
	...
	case'add':
		if(isset($_POST['id']) && $_POST['id'] > 0 && isset($_POST['msg']) && strlen(trim($_POST['msg'])) > 0){
   		    if (isset($_POST['par'])){
                ...
                elseif ($_POST['par'] == 'shop')
                    $comment_id = $COMMENTS->Add(array('shop_id'=>$_POST['id']),$_POST['msg']);
                else
			...
			if($comment_id > 0){
                ...
                if (isset($_POST['par']) && ($_POST['par'] == 'shop')){
                    echo json_encode(array(
                        'html'=>'<div class="comments group" id="comments-'.$comment_id.'-id">
                                    <img src="'.$uInfo['photo'].'">
                                    <div class="text">
                                        <b class="blue-color">'.$uInfo['firstname'].' '.$uInfo['lastname'].'</b>
                                        <em class="grey-light-color">'.ShowDateRus(date('Y-m-d H:i:s')).'</em>
    				...
    				echo json_encode(array('html'=>'
                                <div class="wishlist-comment group" id="comments-'.$comment_id.'-id">
                                    ...
                                    <a href="javascript:;" onclick="CommentsAction('.varr_int($_POST['id']).',\'delete\','.$comment_id.',0)" class="opacity_link fl_r">
                                        <i class="small-icon icon-close"></i>
                                    </a>
                                    <div class="comment-content wrapped">
                                        <a href="/'.$uInfo['user_wp'].'" class="comment-author">'.$uInfo['firstname'].' '.$uInfo['lastname'].'</a><em class="comment-date">'.ShowDateRus(date('Y-m-d H:i:s')).'</em>
                                        <br>
                                        <span class="comment-text">
                                            '.ToText($_POST['msg']).'
                                        </span>
